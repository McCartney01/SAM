...
from sam.common.registry import registry
from sam.models.sam.blip2 import Blip2Base, disabled_train
...
@registry.register_model("sam")
class SAM(Blip2Base):

    PRETRAINED_MODEL_CONFIG_DICT = {
        "sam": "configs/models/sam.yaml"
    }
    ...
    def __init__(
        ...
        from sam.models.sam.modeling_llama import LlamaForCausalLM
        ...
        if freeze_proj:
            for name, param in self.llm_proj.named_parameters():
                param.requires_grad = False
            print("freeze llm proj")

        one_2_muliple = [zero_module(nn.Linear(self.Qformer.config.hidden_size, self.Qformer.config.hidden_size))]
        multiple_2_one = [zero_module(nn.Linear(self.Qformer.config.hidden_size, self.Qformer.config.hidden_size))]
        self.Qformer.set_proj(one_2_muliple, multiple_2_one)

        self.attn = nn.Linear(self.visual_encoder.num_features, 1)
    ...
    def forward(self, samples):
        ...
        if image.dim() == 5:
            ...
            for j in range(image.size(2)):
                ...
                all_image_embeds.append(frame_embeds)
                frame_atts = torch.ones(frame_embeds.size()[:-1], dtype=torch.long).to(image.device)
            
            all_image_embeds = torch.stack(all_image_embeds, dim=0)

            all_image_attn = self.attn(all_image_embeds)
            all_image_attn = nn.functional.softmax(all_image_attn, dim=0)
            avg_image_embeds = (all_image_embeds * all_image_attn).sum(dim=0)

            for j in range(all_image_embeds.size(0)):
                frame_embeds = all_image_embeds[j]
                frame_atts = torch.ones(frame_embeds.size()[:-1], dtype=torch.long).to(image.device)
                ...
                if self.qformer_text_input:
                    frame_query_output = self.Qformer.bert(
                        ...
                        return_dict=True,
                        encoder_hidden_states_2=avg_image_embeds,
                    )
                else:
                    frame_query_output = self.Qformer.bert(
                        query_embeds=query_tokens,
    ...
    def from_config(cls, cfg):
        ...
        if ckpt_path:
            print("Load Proj Checkpoint: {}".format(ckpt_path))
